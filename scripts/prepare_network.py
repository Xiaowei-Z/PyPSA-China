...
import xarray as xr
import yaml
from functions import HVAC_cost_curve, pro_names
...
def get_p_max_pu(path, timerange):
    ...
    pmpu = pd.DataFrame(
        np.load(path),
        index=pd.date_range("1979-01-01 00:00:00", "2016-12-31 23:00:00", freq="H"),
        columns=pro_names,
    )
...
def prepare_costs(Nyears, options):
    ...
    costs.loc[costs.unit.str.contains("tCO2/MWhth"), "value"] *= 1.0e-3
    costs.loc[costs.unit.str.contains("tCO2/MWhth"), "unit"] = "kilo tCO2/MWhth"

    cost_year = 2030

    costs = (
        costs.loc[pd.IndexSlice[:, cost_year, :], "value"]
        .unstack(level=2)
        .groupby(level="technology")
        .sum(min_count=1)
    )
...
def prepare_data(network):
    ...
    intraday_profiles = pd.read_csv(
        "data/heating/heat_load_profile_DK_AdamJensen.csv", index_col=0
    )
    intraday_year_profiles = generate_periodic_profiles(
        dt_index=heat_demand_hdh.index.tz_localize("UTC"),
        weekly_profile=(
            list(intraday_profiles["weekday"]) * 5
            + list(intraday_profiles["weekend"]) * 2
        ),
    ).tz_localize(None)

    space_heat_demand = intraday_year_profiles.mul(heat_demand_hdh).mul(
    ...
    with pd.HDFStore(snakemake.input.co2_totals_name, mode="r") as store:
        co2_totals = 1.0e-3 * Nyears * store["co2"]
...
def prepare_network(options):
    ...
    network.set_snapshots(
        pd.date_range(options["tmin"], options["tmax"], freq=options["freq"])
    )
    ...
    # add carriers
    network.add(
        "Carrier", "gas", co2_emissions=costs.at["gas", "CO2 intensity"]
    )  # in t_CO2/MWht
    ...
    if not isinstance(options["co2_reduction"], tuple):
        ...
        if options["co2_reduction"] is not None:
            ...
            co2_limit *= 1 - options["co2_reduction"]
    ...
    # load demand data
    with pd.HDFStore(
        f'data/load/load_{options["load_year"]}_weatheryears_1979_2016_TWh.h5', mode="r"
    ) as store:
    ...
    network.madd(
        ...
        capital_cost=0.5
        * (costs.at["solar-rooftop", "fixed"] + costs.at["solar-utility", "fixed"]),
    ...
    if options["nuclear"]:
        ...
        network.madd(
            ...
            marginal_cost=costs.at["nuclear", "VOM"]
            + costs.at["uranium", "fuel"] / costs.at["nuclear", "efficiency"],
        )

    if options["add_PHS"]:
        # pure pumped hydro storage, fixed, 6h energy by default, no inflow
        phss = hydrocapa_df.index[hydrocapa_df["p_nom_store[GW]"] > 0].intersection(
            nodes
        )
        ...
        network.madd(
            ...
            carrier="PHS",
            p_nom_extendable=False,
            p_nom=hydrocapa_df.loc[phss]["p_nom_store[GW]"] * 1000.0,  # from GW to MW
    ...
    if options["add_hydro"]:
        ...
        initial_capacity = pd.read_pickle(
            "data/hydro/reservoir_initial_capacity.pickle"
        )
        total_capacity = pd.read_pickle("data/hydro/reservoir_total_capacity.pickle")
        effective_capacity = pd.read_pickle(
            "data/hydro/reservoir_effective_capacity.pickle"
        )
        ...
        #### spillage
        for bus0, bus1 in list(
            zip(dam_buses.iloc[bus0s].index, dam_buses.iloc[bus1s].index)
        ):
            network.add(
                "Link",
    ...
    if options["add_ror"]:
        rors = ror_share.index[ror_share > 0.0]
        rors = rors.intersection(nodes)
        rors = rors.intersection(inflow_df.columns)
        pnom = (
            ror_share[rors] * hydrocapa_df.loc[rors, "p_nom_discharge[GW]"] * 1000.0
        )  # GW to MW
        inflow_pu = inflow_df[rors].multiply(ror_share[rors] / pnom)
        inflow_pu[
            inflow_pu > 1
        ] = 1.0  # limit inflow per unit to one, i.e, excess inflow is spilled here
    ...
    if options["heat_coupling"]:
        ...
        for cat in [" decentral ", " central "]:
            ...
            for dog in [" space ", " water "]:
                ...
                network.madd("Bus", nodes + cat + dog + "heat", carrier="heat")
        ...
        if options["heat_pumps"]:
            ...
            for cat in [" decentral ", " central "]:
                network.madd(
                    ...
                    bus0=nodes,
                    bus1=nodes + cat + "heat",
                    efficiency=ashp_cop[nodes]
                    if options["time_dep_hp_cop"]
                    else costs.at[cat.lstrip() + "air-sourced heat pump", "efficiency"],
                    capital_cost=costs.at[
                        cat.lstrip() + "air-sourced heat pump", "efficiency"
                    ]
                    * costs.at[cat.lstrip() + "air-sourced heat pump", "fixed"],
                    p_nom_extendable=True,
                )
        ...
        if options["retrofitting"]:
            ...
            square_metres = (
                population[retro_nodes] / population["DE"] * 5.7e9
            )  # HPI 3.4e9m^2 for DE res, 2.3e9m^2 for tert https://doi.org/10.1016/j.rser.2013.09.012
            ...
            network.madd(
                ...
                p_nom_extendable=True,
                p_nom_max=options["retroI-fraction"]
                * space_peak
                * (1 - central_fraction),
                p_max_pu=space_pu,
                p_min_pu=space_pu,
                capital_cost=options["retrofitting-cost_factor"]
                * costs.at["retrofitting I", "fixed"]
                * square_metres
                / (options["retroI-fraction"] * space_peak),
            )
            ...
            network.madd(
                ...
                carrier="retrofitting",
                p_nom_extendable=True,
                p_nom_max=options["retroII-fraction"]
                * space_peak
                * (1 - central_fraction),
                p_max_pu=space_pu,
                p_min_pu=space_pu,
                capital_cost=options["retrofitting-cost_factor"]
                * costs.at["retrofitting II", "fixed"]
                * square_metres
                / (options["retroII-fraction"] * space_peak),
            )
        ...
        if options["tes"]:
            ...
            for cat in [" decentral ", " central "]:
                network.madd("Bus", nodes + cat + "water tanks", carrier="water tanks")
                ...
                network.madd(
                    ...
                    - np.exp(
                        ...
                        / (
                            24.0
                            * (options["tes_tau"] if cat == " decentral " else 180.0)
                        )
                    ),  # [HP] 180 day time constant for centralised, 3 day for decentralised
                    capital_cost=costs.at[cat.lstrip() + "water tank storage", "fixed"]
                    / (1.17e-3 * 40),
        ...
        if options["boilers"]:
            ...
            for cat in [" decentral ", " central "]:
                ...
                network.madd("Bus", nodes + cat + "gas", carrier="gas")
                ...
                network.madd(
                    ...
                    * costs.at[cat.lstrip() + "gas boiler", "fixed"],
                )

        if options["chp"]:

            network.madd("Bus", nodes + " CHPgas", carrier="gas")
            ...
            network.madd("Bus", nodes + " CHPcoal", carrier="coal")
        ...
        if options["solar_thermal"]:
            ...
            with pd.HDFStore(snakemake.input.solar_thermal_name, mode="r") as store:
                ...
                solar_thermal = (
                    options["solar_cf_correction"]
                    * store["solar_thermal_profiles"]
                    / 1e3
                )
    ...
    if options["dac"]:  # Direct Air Capture
        ...
        network.add("Bus", "EU co2", carrier="co2")

        # could add capital costs here
        network.add("Store", "EU co2 Store", bus="EU co2", e_nom_extendable=True)
    ...
    if options["transport_coupling"]:
        ...
        if options["bev"]:
            ...
            network.madd(
                ...
                bus=nodes + " EV battery",
                e_cyclic=True,
                e_nom=transport_data["number cars"]
                * 0.05
    ...
    if not network.options["no_lines"]:
        ...
        if options["line_volume_limit_max"] is not None:
            ...
            cc = (
                (
                    options["line_cost_factor"]
                    * lengths
                    * [HVAC_cost_curve(l) for l in lengths]
                )
                * 1.5
                * 1.02
                * Nyears
                * annuity(40.0, options["discountrate"])
            )
...
if __name__ == "__main__":
    ...
    if "snakemake" not in globals():
        ...
        snakemake.input = Dict(
            options_name=config["results_dir"]
            + "version-"
            + str(config["version"])
            + "/options/options-{flexibility}-{line_limits}-{co2_reduction}.yml",
            population_name="data/population/population.h5",
            solar_thermal_name="data/heating/solar_thermal-{angle}.h5".format(
                angle=config["solar_thermal_angle"]
            ),
        ...
        snakemake.output = Dict(
            ...
            + "version-"
            + str(config["version"])
            + "/prenetworks/prenetwork-{flexibility}-{line_limits}-{co2_reduction}.nc"
        )
