...
if __name__ == "__main__":
    ...
    cutout.prepare()
    provinces_shp = gpd.read_file(snakemake.input["provinces_shp"])[
        ["NAME_1", "geometry"]
    ]
    ...
    if snakemake.config["Technique"]["solar"]:
        ...
        if noprogress:
            ...
            solar_matrix = cutout.availabilitymatrix(
                provinces_shp, excluder_solar, **kwargs
            )
            buildup_matrix = cutout.availabilitymatrix(
                provinces_shp, excluder_build_up, **kwargs
            )
        ...
        solar_potential = (
            solar_capacity_per_sqkm * 0.7 * solar_matrix.sum("bus") * area
            + solar_capacity_per_sqkm * 0.3 * buildup_matrix.sum("bus") * area
        )
        ...
        solar_ds = solar_ds.sel(
            bus=(
                (
                    solar_ds["profile"].mean("time")
                    > solar_config.get("min_p_max_pu", 0.0)
                )
        ...
        if "clip_p_max_pu" in solar_config:
            ...
            solar_ds["profile"] = solar_ds["profile"].where(
                solar_ds["profile"] >= min_p_max_pu, 0
            )
    ...
    if snakemake.config["Technique"]["onwind"]:
        ...
        if onwind_correction_factor != 1.0:
            logger.info(
                f"onwind_correction_factor is set as {onwind_correction_factor}"
            )
        ...
        if noprogress:
            ...
            start = time.time()
            onwind_matrix = cutout.availabilitymatrix(
                provinces_shp, excluder_onwind, **kwargs
            )
            duration = time.time() - start
        ...
        onwind_ds = onwind_ds.sel(
            bus=(
                (
                    onwind_ds["profile"].mean("time")
                    > onwind_config.get("min_p_max_pu", 0.0)
                )
        ...
        if "clip_p_max_pu" in onwind_config:
            ...
            onwind_ds["profile"] = onwind_ds["profile"].where(
                onwind_ds["profile"] >= min_p_max_pu, 0
            )
    ...
    if snakemake.config["Technique"]["offwind"]:
        ...
        offwind_correction_factor = offwind_config.get("correction_factor", 1.0)
        offwind_capacity_per_sqkm = offwind_config["capacity_per_sqkm"]
        if offwind_correction_factor != 1.0:
            logger.info(
                f"offwind_correction_factor is set as {offwind_correction_factor}"
            )

        offwind_pro_names = np.array(
            [
                "Fujian",
                "Guangdong",
                "Guangxi",
        ...
        EEZ_province_shp = gpd.read_file(
            snakemake.input["offshore_province_shapes"]
        ).set_index("index")
        EEZ_province_shp = EEZ_province_shp.reindex(offwind_pro_names).rename_axis(
            "bus"
        )
        ...
        if "max_depth" in offwind_config:
            ...
            excluder_offwind.add_raster(
                snakemake.input.gebco, codes=func, crs=4236, nodata=-1000
            )
        ...
        if offwind_config["natura"]:
            ...
            Protected_Marine_shp = gpd.tools.overlay(
                Protected_shp, EEZ_shp, how="intersection"
            )
        ...
        if noprogress:
            ...
            start = time.time()
            offwind_matrix = cutout.availabilitymatrix(
                EEZ_province_shp, excluder_offwind, **kwargs
            )
            duration = time.time() - start
            logger.info(
                f"Completed offwind availability calculation ({duration:2.2f}s)"
            )
        else:
            offwind_matrix = cutout.availabilitymatrix(
                EEZ_province_shp, excluder_offwind, **kwargs
        ...
        offwind_ds = offwind_ds.sel(
            bus=(
                (
                    offwind_ds["profile"].mean("time")
                    > offwind_config.get("min_p_max_pu", 0.0)
                )
