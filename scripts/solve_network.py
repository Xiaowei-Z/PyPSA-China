...
def extra_functionality(network,snapshots):
    ...
    def define_co2_el_and_th_constraints(network,snapshots):
        ...
        el_gas_stores = network.stores.index[([('gas Store' in s) & ('heat' not in s) for s in network.stores.index]) & (~network.stores.e_cyclic)]
        heat_gas_stores = network.stores.index[(['heat gas Store' in s for s in network.stores.index]) & (~network.stores.e_cyclic)]

        CHP_gas_stores = network.stores.index[(['CHPgas Store' in s for s in network.stores.index]) & (~network.stores.e_cyclic)]

        co2_el_and_th_constraints = {}
        co2_el_and_th_constraints_list = ['co2_limit_el', 'co2_limit_th']

        gas_stores = {co2_el_and_th_constraints_list[0]:el_gas_stores, co2_el_and_th_constraints_list[1]:heat_gas_stores}
        ...
        for gc in co2_el_and_th_constraints_list:
            ...
            for carrier in network.carriers.index:
                ...
                if options['CHP_emission_accounting']=='200percent':
                    ...
                    if gc == 'co2_limit_th':
                        # 0.5 * Pth
                        c.lhs.variables.extend([(attribute*0.5*options['chp_parameters']['eta_elec']/options['chp_parameters']['c_v'], network.model.link_p[s.split()[0] + " central CHP heat",sn]) for sn in snapshots for s in CHP_gas_stores])
                    elif gc == 'co2_limit_el':
                        # Fuel - Pth
                        c.lhs.variables.extend([(attribute, network.model.store_p[s,sn]) for sn in snapshots for s in CHP_gas_stores])
                        c.lhs.variables.extend([(-attribute*0.5*options['chp_parameters']['eta_elec']/options['chp_parameters']['c_v'], network.model.link_p[s.split()[0] + " central CHP heat",sn]) for sn in snapshots for s in CHP_gas_stores])
                ...
                if options['CHP_emission_accounting']=='dresden':
                    ...
                    if gc == 'co2_limit_el':
                        ...
                        c.lhs.variables.extend([(attribute, network.model.link_p[s.split()[0] + " central CHP electric",sn]) for sn in snapshots for s in CHP_gas_stores])
                ...
                if options['CHP_emission_accounting']=='substitution':
                    ...
                    if gc == 'co2_limit_el':
                        ...
                        c.lhs.variables.extend([(attribute*0.5, network.model.link_p[s.split()[0] + " central CHP electric",sn]) for sn in snapshots for s in CHP_gas_stores])
                        c.lhs.variables.extend([(-attribute*0.5/(eta_gas_boiler*options['chp_parameters']['eta_elec']/options['chp_parameters']['c_v']), network.model.link_p[s.split()[0] + " central CHP heat",sn]) for sn in snapshots for s in CHP_gas_stores])
                        c.lhs.variables.extend([(0.5*attribute, network.model.store_p[s,sn]) for sn in snapshots for s in CHP_gas_stores])
                    elif gc == 'co2_limit_th':
        ...
        pypsa.opt.l_constraint(network.model, "co2_el_and_th_constraints",
                     co2_el_and_th_constraints, co2_el_and_th_constraints_list)
...
def solve_model(network):
    ...
    if options['line_volume_limit_factor'] is not None:
        network.shadow_prices["line_volume_limit"] = network.model.dual[getattr(network.model, 'line_volume_limit')]

    if not isinstance(options['co2_reduction'], tuple):
        if options['co2_reduction'] is not None:
            network.shadow_prices['co2_constraint'] = network.global_constraints.loc["co2_limit","mu"]
    else:
        duals = pd.Series(list(network.model.dual.values()), index=pd.Index(list(network.model.dual.keys())))
        network.shadow_prices["co2_constraint"] = - get_shadows(network.model.co2_el_and_th_constraints, multiind=False)
...
if __name__ == '__main__':
    ...
    with pd.HDFStore(snakemake.input.co2_totals_name, mode='r') as store:
        network.co2_totals = 1.e-3 * Nyears * store['co2']
